...
ZEND_METHOD(HtmlInspector, entities_to_utf8)
{
    ...
    HtmlInspector_entities_to_utf8(&string);
...
{
    ZEND_PARSE_PARAMETERS_NONE();
    zend_object *fork = zend_objects_clone_obj(Z_OBJ_P(ZEND_THIS));
    ZVAL_OBJ(return_value, fork);
    HTML_INSPECTOR(return_value) = HtmlInspector_fork(HTML_INSPECTOR(ZEND_THIS));
    if (HTML_INSPECTOR(return_value) == NULL) {
        zend_throw_error(NULL, "malloc failed", 0);
    }
}
...
ZEND_METHOD(HtmlInspector, reset)
{
    ...
    Z_ADDREF_P(ZEND_THIS);  // Prevents double __destruct call and thus segmentation fault
...
ZEND_BEGIN_ARG_INFO(arginfo_nth, 0)
...
ZEND_BEGIN_ARG_INFO(arginfo_child, 0)
ZEND_END_ARG_INFO()
...
ZEND_BEGIN_ARG_INFO(arginfo_or, 0)
ZEND_END_ARG_INFO()
...
ZEND_BEGIN_ARG_INFO(arginfo_and, 0)
ZEND_END_ARG_INFO()
...
ZEND_BEGIN_ARG_INFO(arginfo_not, 0)
ZEND_END_ARG_INFO()
...
ZEND_BEGIN_ARG_INFO(arginfo_case_i, 0)
ZEND_END_ARG_INFO()

ZEND_METHOD(HtmlInspector, get_name)
{
    ZEND_PARSE_PARAMETERS_NONE();
    struct String string = HtmlInspector_get_name(HTML_INSPECTOR(ZEND_THIS));
    if (string.data == NULL) {
        ZVAL_NULL(return_value);
        return;
    }
