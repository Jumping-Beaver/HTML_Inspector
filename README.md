...
This is a readonly error-tolerant HTML parser and IRI/URI resolver with focus on fast
performance. Data queries are supported by iterating over node axes while applying boolean filters,
similar to CSS or XPath selectors. The use case is very large scale web crawling. The parser is
implemented in C.

## Usage

[PHP bindings are here.](https://codeberg.org/Jumping-Beaver/HTML_Inspector_for_PHP)
...
- Provide a function to resolve URLs
...
- Implementation of a pure XML parser
...
factor of 10. LibXML2 does not parse the contents of script, title, style and textarea tags
correctly, does not support case insensitive attribute names, and handles the input encoding in a
way that is hard to control. Gumbo is not really maintained. Lexbor is a recommendable alternative;
it has more features, has a wider scope than just web scraping, has slower performance and might be
less maintainable due to its larger code base.
...
Another complication are optional starting and closing tags. They require a large amount of code to
handle.
...
allocator enhanced the performance significantly, it is not satisfactory: from 15 parsings / second
to 20 parsings / seconds.

### Examples of defects in Python's `lxml` module
